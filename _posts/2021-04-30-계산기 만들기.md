...
<script type="text/javascript">
	function num(no) {
		...
		var z = document.getElementById("txt4");
		var r = document.getElementById("txt5");
		if (t.value == 0) {
			t.value = no;
		} else {
			t.value += no;
	...
	function calc(ca) {
		...
		if (x.value == '') { // 이전 부호값이 없을 때
			y.value = t.value;
		} else { // 이전 부호값이 있을 때
			switch (x.value) {
			case '+': // 기존 부호값이 +일 때
				y.value = (y.value * 1) + (t.value * 1);
				z.value = t.value;
				r.value = x.value;
				break;
			case '-': // 기존 부호값이 -일 때
		...
		x.value = ca; // 부호값에 누른 부호를 넣어준다
		t.value = 0; // 입력값을 0으로 변경한다
	}
...
<body>
	...
	<input type="text" id="txt3" value="0" readonly="readonly" />
	<input type="text" id="txt4" value="0" readonly="readonly" />
	<input type="text" id="txt5" value="" readonly="readonly" />
	<br />
	...
	<input type="button" value="c" onclick="can('c');" />
	<input type="button" value="=" onclick="calc('=');" />
	<input type="button" value="/" onclick="calc('/');" />
